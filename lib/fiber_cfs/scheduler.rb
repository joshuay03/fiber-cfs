...
module FiberCFS
  class Scheduler
    ...
    def io_blocking io, buffer, from, length, offset, interest = nil
      total = 0
      method, event = case interest
      when :r then [:pread, IO::READABLE]
      when :w then [:pwrite, IO::WRITABLE]
      else raise ArgumentError, "invalid interest: #{interest}"
      end

      while true
        result = blocking { buffer.send method, io, from, 0, offset }
        ...
        if result > 0
          ...
          break
        elsif result == -Errno::EAGAIN::Errno
          if length > 0
            io_wait io, event, nil
          else
            return result
          end
        elsif result < 0
    ...
    module Implementation
      ...
      def block _blocker, timeout = nil
        ...
        @blocked << blocked_node = (BlockedNode.new NodeData.new **data_attrs)
      ...
      def io_pread io, buffer, from, length, offset
        ...
        io_blocking io, buffer, from, length, offset, :r
      end
      ...
      def io_pwrite io, buffer, from, length, offset
        ...
        io_blocking io, buffer, from, length, offset, :w
      end
      ...
      def io_read io, buffer, length, offset
        ...
        io_blocking io, buffer, 0, length, offset, :r
      end
      ...
      def io_write io, buffer, length, offset
        ...
        io_blocking io, buffer, 0, length, offset, :w
      end
