...
puts (3.times.map do
  Benchmark.measure do
    ...
    def run_fibers_in_thread(fiber_count, thread_id)
      ...
      (fiber_count / 2).times do |i|
        Fiber.schedule do
          io_bound_work("IO Fiber #{thread_id}-#{Fiber.current.object_id}", rand(2..5))
        end
      end

      Fiber.schedule do
        io_read_work("IO Fiber #{thread_id}-#{Fiber.current.object_id}", rand(2..5))
      end
    ...
    thread1 = Thread.new { set_scheduler!; run_fibers_in_thread(5, 1) }
    # thread2 = Thread.new { set_scheduler!; run_fibers_in_thread(5, 2) }
